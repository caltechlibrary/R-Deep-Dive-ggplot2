...
2. `aes()`: these are the **`aes`thetic** mappings for the plot - how we are visually representing the data variables. The basic ones used here are:
...
Recreate your graph from Exercise 1, but add the following to the *main data layer* aesthetics, 
...
```{r section_3-int_to_factor, eval=F, echo=T}
periodicTable$period <- factor(periodicTable$period)
```

Then, when you add `color=period`, ggplot will automatically plot it as 
a factor.
...
pressing *Cmd+Option+I*. Recreate the graph from Exercise 4 and label the X-axis "Element Type", 
Y-axis "Number of Elements", and the plot title "Numbers of Elements by Type". 
