...
```{r adding_color_to_periodicity_factors}
# Remove this for the learner version.
...
Let's go back to our original graph of mass versus numb from the beginning of Section 3 (the plot_for_data_integrity chunk, if you are using the navigation menu at the bottom):
...
and using this in the label option. Run the follwoing code for an example:
...
Add a new chunk by clicking the *Insert Chunk* button (top of the window on the toolbar) or by pressing *Cmd+Option+I*. Based on what we've done so far, create a bar plot that shows how many elements there are of each phase. Label with an appropriate title and appropriate axes names. As a bonus, omit the NA data.
...
# 7 - Three-Dimensional Plots: Periodic Table Trends
